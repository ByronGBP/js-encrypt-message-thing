...
class Clever {
  talk (input) {
    let output = '';
    if (input.length > 0 && Number(input[0]) >= '0' && Number(input[input.length - 1]) <= '9') {
      const arr = input.split(',');
      output = this._decryptMessage(arr);
    } else if (input.length > 0) {
      output = this._encryptMessae(input);
    }
